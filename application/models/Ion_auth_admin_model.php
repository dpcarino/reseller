 ...
 */
class Ion_auth_admin_model extends CI_Model
{
	/**
	...
	public function __construct()
	{
		...
		$this->db = $this->load->database($this->config->item('database_group_name', 'ion_auth_admin'), TRUE, TRUE);

		// initialize db tables data
		$this->tables = $this->config->item('tables', 'ion_auth_admin');

		// initialize data
		$this->identity_column = $this->config->item('identity', 'ion_auth_admin');
		$this->store_salt = $this->config->item('store_salt', 'ion_auth_admin');
		$this->salt_length = $this->config->item('salt_length', 'ion_auth_admin');
		$this->join = $this->config->item('join', 'ion_auth_admin');

		// initialize hash method options (Bcrypt)
		$this->hash_method = $this->config->item('hash_method', 'ion_auth_admin');
		$this->default_rounds = $this->config->item('default_rounds', 'ion_auth_admin');
		$this->random_rounds = $this->config->item('random_rounds', 'ion_auth_admin');
		$this->min_rounds = $this->config->item('min_rounds', 'ion_auth_admin');
		$this->max_rounds = $this->config->item('max_rounds', 'ion_auth_admin');

		// initialize messages and error
		$this->messages    = array();
		$this->errors      = array();
		$delimiters_source = $this->config->item('delimiters_source', 'ion_auth_admin');
		...
		else
		{
			...
			$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'ion_auth_admin');
			$this->message_end_delimiter = $this->config->item('message_end_delimiter', 'ion_auth_admin');
			$this->error_start_delimiter = $this->config->item('error_start_delimiter', 'ion_auth_admin');
			$this->error_end_delimiter = $this->config->item('error_end_delimiter', 'ion_auth_admin');
		}
		...
		if ($this->hash_method == 'bcrypt')
		{
			...
			$params['salt_prefix'] = $this->config->item('salt_prefix', 'ion_auth_admin');
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'ion_auth_admin');
		...
		else if (!$this->config->item('default_group', 'ion_auth_admin') && empty($groups))
		{
		...
		// $query = $this->db->get_where($this->tables['groups'], array('name' => $this->config->item('default_group', 'ion_auth_admin')), 1)->row();
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				if ($remember && $this->config->item('remember_users', 'ion_auth_admin'))
				{
	...
	{
		$recheck = (NULL !== $this->config->item('recheck_timer', 'ion_auth_admin')) ? $this->config->item('recheck_timer', 'ion_auth_admin') : 0;
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_admin'))
		{
			$max_attempts = $this->config->item('maximum_login_attempts', 'ion_auth_admin');
			if ($max_attempts > 0)
			{
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_admin'))
		{
			...
			if ($this->config->item('track_login_ip_address', 'ion_auth_admin'))
			{
				if (!isset($ip_address))
				{
					$ip_address = $this->_prepare_ip($this->input->ip_address());
			...
			$this->db->where('time >', time() - $this->config->item('lockout_time', 'ion_auth_admin'), FALSE);
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_admin') && $this->config->item('track_login_ip_address', 'ion_auth_admin'))
		{
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_admin'))
		{
			...
			$old_attempts_expire_period = max($old_attempts_expire_period, $this->config->item('lockout_time', 'ion_auth_admin'));
	...
	public function set_lang($lang = 'en')
	{
		...
		if($this->config->item('user_expire', 'ion_auth_admin') === 0)
		{
		...
		{
			$expire = $this->config->item('user_expire', 'ion_auth_admin');
		}
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => $this->config->item('identity_cookie_name', 'ion_auth_admin'),
			    'value'  => $user->{$this->identity_column},
			    'expire' => $expire
			));

			set_cookie(array(
			    'name'   => $this->config->item('remember_cookie_name', 'ion_auth_admin'),
			    'value'  => $salt,
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		if (!get_cookie($this->config->item('identity_cookie_name', 'ion_auth_admin'))
			|| !get_cookie($this->config->item('remember_cookie_name', 'ion_auth_admin'))
			|| !$this->identity_check(get_cookie($this->config->item('identity_cookie_name', 'ion_auth_admin'))))
		{
		...
		$query = $this->db->select($this->identity_column . ', id, email, last_login')
						  ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name', 'ion_auth_admin'))))
						  ->where('remember_code', get_cookie($this->config->item('remember_cookie_name', 'ion_auth_admin')))
						  ->where('active', 1)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->config->item('user_extend_on_login', 'ion_auth_admin'))
			{
				$this->remember_user($user->id);
			}
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if ($this->config->item('admin_group', 'ion_auth_admin') === $group->name && $group_name !== $group->name)
		{
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'ion_auth_admin'))
		{
