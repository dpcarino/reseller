 ...
 */
class Ion_auth_member_model extends CI_Model
{
	/**
	...
	{
		$this->config->load('ion_auth_member', TRUE);
		$this->load->helper('cookie');
		$this->load->helper('date');
		$this->lang->load('ion_auth_member');

		// initialize the database
		$this->db = $this->load->database($this->config->item('database_group_name', 'ion_auth_member'), TRUE, TRUE);

		// initialize db tables data
		$this->tables = $this->config->item('tables', 'ion_auth_member');

		// initialize data
		$this->identity_column = $this->config->item('identity', 'ion_auth_member');
		$this->store_salt = $this->config->item('store_salt', 'ion_auth_member');
		$this->salt_length = $this->config->item('salt_length', 'ion_auth_member');
		$this->join = $this->config->item('join', 'ion_auth_member');

		// initialize hash method options (Bcrypt)
		$this->hash_method = $this->config->item('hash_method', 'ion_auth_member');
		$this->default_rounds = $this->config->item('default_rounds', 'ion_auth_member');
		$this->random_rounds = $this->config->item('random_rounds', 'ion_auth_member');
		$this->min_rounds = $this->config->item('min_rounds', 'ion_auth_member');
		$this->max_rounds = $this->config->item('max_rounds', 'ion_auth_member');

		// initialize messages and error
		$this->messages    = array();
		$this->errors      = array();
		$delimiters_source = $this->config->item('delimiters_source', 'ion_auth_member');
		...
		else
		{
			...
			$this->message_start_delimiter = $this->config->item('message_start_delimiter', 'ion_auth_member');
			$this->message_end_delimiter = $this->config->item('message_end_delimiter', 'ion_auth_member');
			$this->error_start_delimiter = $this->config->item('error_start_delimiter', 'ion_auth_member');
			$this->error_end_delimiter = $this->config->item('error_end_delimiter', 'ion_auth_member');
		}
		...
		if ($this->hash_method == 'bcrypt')
		{
			...
			$params['salt_prefix'] = $this->config->item('salt_prefix', 'ion_auth_member');
	...
	public function hash_security_pin_db($id, $security_pin, $use_sha1_override = FALSE)
	{
		if (empty($id) || empty($security_pin))
		{
			return FALSE;
		}
		...
		$query = $this->db->select('security_pin, salt')
		                  ->where('member_id', $id)
		                  ->limit(1)
		                  ->order_by('member_id', 'desc')
		                  ->get($this->tables['users']);

		$hash_security_pin_db = $query->row();
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			if ($this->bcrypt->verify($security_pin,$hash_security_pin_db->security_pin))
			{
				return TRUE;
			}
		...
		if ($this->store_salt)
		{
			$db_security_pin = sha1($security_pin . $hash_security_pin_db->salt);
		}
		else
		{
			$salt = substr($hash_security_pin_db->security_pin, 0, $this->salt_length);

			$db_security_pin =  $salt . substr(sha1($salt . $security_pin), 0, -$this->salt_length);
		}

		if($db_security_pin == $hash_security_pin_db->security_pin)
		{
			return TRUE;
		}
		else
	...
	public function deactivate($id = NULL)
	{
		...
		else if ($this->ion_auth_member->logged_in() && $this->user()->row()->member_id == $id)
		{
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('member_id, password, salt')
	...
	public function forgotten_password_complete($code, $salt = FALSE)
	{
		...
		if ($profile)
		{
			...
			if ($this->config->item('forgot_password_expiration', 'ion_auth_member') > 0)
			{
				//Make sure it isn't expired
				$expiration = $this->config->item('forgot_password_expiration', 'ion_auth_member');
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$manual_activation = $this->config->item('manual_activation', 'ion_auth_member');
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, member_id, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->where('group_id', 3)
						  ->limit(1)
						  ->order_by('member_id', 'desc')
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				if ($remember && $this->config->item('remember_users', 'ion_auth_member'))
				{
	...
	{
		$recheck = (NULL !== $this->config->item('recheck_timer', 'ion_auth_member')) ? $this->config->item('recheck_timer', 'ion_auth_member') : 0;
		...
		if ($recheck !== 0)
		{
			...
			if ($last_login + $recheck < time())
			{
				...
				else
				{
					...
					$identity = $this->config->item('identity', 'ion_auth_member');

					if (substr(CI_VERSION, 0, 1) == '2')
					{
						$this->session->unset_userdata(array($identity => '', 'member_id' => '', 'member_id' => ''));
					}
					else
					{
						$this->session->unset_userdata(array($identity, 'member_id', 'member_id'));
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_member'))
		{
			$max_attempts = $this->config->item('maximum_login_attempts', 'ion_auth_member');
			if ($max_attempts > 0)
			{
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_member'))
		{
			...
			if ($this->config->item('track_login_ip_address', 'ion_auth_member'))
			{
				if (!isset($ip_address))
				{
					$ip_address = $this->_prepare_ip($this->input->ip_address());
			...
			$this->db->where('time >', time() - $this->config->item('lockout_time', 'ion_auth_member'), FALSE);
	...
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_member') && $this->config->item('track_login_ip_address', 'ion_auth_member'))
		{
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth_member'))
		{
			...
			$old_attempts_expire_period = max($old_attempts_expire_period, $this->config->item('lockout_time', 'ion_auth_member'));
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.member_id as member_id',
			    $this->tables['users'].'.member_id as member_id'
			));
		}
	...
	public function user($id = NULL)
	{
		...
		$id = isset($id) ? $id : $this->session->userdata('member_id');
	...
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('member_id');
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => date('Y-m-d H:i:s')), array('member_id' => $id));
	...
	public function set_lang($lang = 'en')
	{
		...
		if($this->config->item('user_expire', 'ion_auth_member') === 0)
		{
		...
		{
			$expire = $this->config->item('user_expire', 'ion_auth_member');
		}
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'email'                => $user->email,
		    'member_id'            => $user->member_id, //everyone likes to overwrite id so we'll use user_id
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => $this->config->item('identity_cookie_name_member', 'ion_auth_member'),
			    'value'  => $user->{$this->identity_column},
			    'expire' => $expire
			));

			set_cookie(array(
			    'name'   => $this->config->item('remember_cookie_name_member', 'ion_auth_member'),
			    'value'  => $salt,
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		if (!get_cookie($this->config->item('identity_cookie_name_member', 'ion_auth_member'))
			|| !get_cookie($this->config->item('remember_cookie_name_member', 'ion_auth_member'))
			|| !$this->identity_check(get_cookie($this->config->item('identity_cookie_name_member', 'ion_auth_member'))))
		{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', member_id, email, last_login')
						  ->where($this->identity_column, urldecode(get_cookie($this->config->item('identity_cookie_name_member', 'ion_auth_member'))))
						  ->where('remember_code', get_cookie($this->config->item('remember_cookie_name_member', 'ion_auth_member')))
						  ->where('active', 1)
						  ->limit(1)
						  ->order_by('member_id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() == 1)
		{
			...
			if ($this->config->item('user_extend_on_login', 'ion_auth_member'))
			{
				$this->remember_user($user->member_id);
			}
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if ($this->config->item('admin_group', 'ion_auth_member') === $group->name && $group_name !== $group->name)
		{
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if($group->name == $this->config->item('admin_group', 'ion_auth_member'))
		{
